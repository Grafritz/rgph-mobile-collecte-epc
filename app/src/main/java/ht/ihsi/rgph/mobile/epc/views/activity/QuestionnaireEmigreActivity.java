 ...
 */
public class QuestionnaireEmigreActivity extends BaseActivity  implements Serializable, View.OnClickListener, TextView.OnEditorActionListener , AdapterView.OnItemSelectedListener{
    //region ATTRIBUTS
...
//
            ...
            QF = new QuestionnaireFormulaireUtility();
            QF = (QuestionnaireFormulaireUtility) intent.getSerializableExtra(Constant.PARAM_QUESTIONNAIRE_FORMULAIRE);
            QF.context = QuestionnaireEmigreActivity.this;
        ...
        try{
            switch (item.getItemId()){
                case R.id.btn_actionbar_SaveAndQuit:
                    try {
                        QF.CheckValueBefore(queryRecordMngr, cuRecordMngr, Constant.SetValueTempInfoQuestion_Emigre, et_Reponse, sp_Reponse, sp_Reponse2, sp_Reponse3, sp_Jour, sp_Mois, sp_Annee
                        ...
                        QuestionnaireMenageActivity.CounterForEmigre += 1;
                        QF.context = QuestionnaireEmigreActivity.this;
                        finishAfter=true;
    ...
    public boolean onKeyDown(final int keyCode, final KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK){
            ...
            builder.setMessage("" + getString(R.string.msg_Eske_Ou_Vle_Kite___) + " Emigre a vre.")
                    .setCancelable(false)
                    ...
                    .setNegativeButton("" + getString(R.string.label_WI), new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                            QuestionnaireEmigreActivity.super.onKeyDown(_keyCode, _event);
                            CreateMenuContext();
                        }
                    });
    ...
    //region PopUp Add
    private void AddEmigrer_EnBoucle() {
        try {
            ...
            if (NbreTotalEmigre > 0) {
                ...
                if (NbreTotalEmigre == nbreTotalEmigre_DejaSave) {
                    // On lui permet de voir la liste des Emigres deja enregistrer.
                    // Ici on doit Afficher le formulaire du premier Emigre qui n'est pas encore fini
                    // Et qui est soit remplit Totalement ou pas
                    if( QuestionnaireMenageActivity.CounterForEmigre >= NbreTotalEmigre  ) {
                        finish();
                    }else{
                        ...
                        do{
                            emigM = queryRecordMngr.searchEmigre_ByNoOrdre_ByIdMenage( QuestionnaireMenageActivity.CounterForEmigre, QF.getMenageModel().getMenageId() );
                            QuestionnaireMenageActivity.CounterForEmigre += 1;
                        }while (emigM == null && QuestionnaireMenageActivity.CounterForEmigre <= NbreTotalEmigre );
                        ...
                        if ( emigM != null ) {
                            ...
                            emigreModel.setDateDebutCollecte(dateDebutCollect.toString());
                            emigreModel = emigM;

                                    // Objet Batimtnent
                            emigreModel.setBatimentId(QF.getMenageModel().getBatimentId());
                            emigreModel.setObjBatiment(QF.getBatimentModel());
                            // Objet Logement
                            emigreModel.setLogeId(QF.getMenageModel().getLogeId());
                            emigreModel.setObjLogement(QF.getLogementModel());
                            // Objet Menage
                            emigreModel.setMenageId(QF.getMenageModel().getMenageId());
                            emigreModel.setObjMenage(QF.getMenageModel());

                            headerFormOne = "MODIFYE EMIGRE " + nbr_ou_NoOrdre + "/" + NbreTotalEmigre;
                            ...
                            message = "Kontinye pran enfòmasyon sou Emigre " + nbr_ou_NoOrdre + " a";
                    ...
                    headerFormOne = "NOUVO EMIGRE " + nbr_ou_NoOrdre + "/" + NbreTotalEmigre;
                    headerFormTwo = " Menaj " + QF.getMenageModel().getQm1NoOrdre()
                            + " | Lojman Endividyèl " + QF.getLogementModel().getQlin1NumeroOrdre()
                            + " | Batiman " + QF.getLogementModel().getBatimentId();// + " | REC: " + QF.getLogementModel().getObjBatiment().getQrec();
                    ...
                    message = "Kòmanse pran enfòmasyon sou Emigre " + nbr_ou_NoOrdre + " a";
                    ToastUtility.ToastMessage(this, message );
                    Tools.AlertDialogMsg(this, message, "S");
                }
            }
        ...
        return new RadioListAdapter.OnItemClickListener(){
            ...
            public void onItemClick(QuestionReponseModel  questionReponseModel) {
                ...
                if (codeReponseRecyclerView != null) {
                    ...
                    QF.SetButtonLabel(QuestionnaireEmigreActivity.this, codeReponseRecyclerView.getCodeReponse(), QF, btn_Suivant);
                }
                message = questionReponseModel.toString();
                ToastUtility.ToastMessage(QuestionnaireEmigreActivity.this, message, Constant.GravityCenter);
                Suivant_Click();
            }
        };
    }
    ...
    public void Suivant_Click() {
        try {
            ...
            if (QF.getqSuivant().toString().equalsIgnoreCase(Constant.FIN)) {
                ...
                // On met le boucle ici pour les Emigres
                //AddEmigrer_EnBoucle();

                int statut = Constant.STATUT_RANPLI_NET_11;//QF.SetStatutEmigrer(queryRecordMngr, cuRecordMngr);wwwwwwwwwwwwww
                        ...
                        switch (which) {
                            case 0:// label_SaveAndQuit
                                try {
                                    ...
                                    Tools.AlertDialogMsg(QuestionnaireEmigreActivity.this, ex.getMessage());
                                    ...
                                    ToastUtility.ToastMessage(QuestionnaireEmigreActivity.this, ex.getMessage());
                                } catch (Exception ex) {
                                    ToastUtility.LogCat("Exception: CreateMenuContext() :" + ex.getMessage() + " / toString: " + ex.toString());
                                    ex.printStackTrace();
                                }
    ...
    public void ShowRapport_RAR(int statut) {
        try {
            if( QF.TypeEvenement != Constant.ACTION_AFFICHER ) {
                ...
                message = String.format(getString(R.string.label_msgInfoRapport), " Emigre ", " Menaj " + QF.getMenageModel().getQm1NoOrdre());
                ...
                message = "" + "<b>Rapò sou Emigre </b>";
                if (QF.getEmigreModel().getQn1numeroOrdre() != null) {
                    message += "" + "<b> " + QF.getEmigreModel().getQn1numeroOrdre() + "</b>";
                }
                ...
                btnContinuer.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        try {
                            Shared_Preferences sharedPreferences = Tools.SharedPreferences(QuestionnaireEmigreActivity.this);
                            ...
                            if (finishAfter) {
                                ...
                                // On met le boucle ici pour l'Emigrer
                                AddEmigrer_EnBoucle();
                            }
                        } catch (TextEmptyException ex) {
                            Tools.AlertDialogMsg(QuestionnaireEmigreActivity.this, ex.getMessage());
